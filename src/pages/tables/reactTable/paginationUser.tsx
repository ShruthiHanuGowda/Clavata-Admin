...
import { UserTableRow } from 'types/table';
...
export default function PaginationUserTable() {
   ...
   *
   * GraphQL has been completely removed from this component.
   *
   * Once you have your REST/API endpoint ready, replace this
   * with the API response.
   */
  const [data] = useState<UserTableRow[]>([]);
  ...
  /**
   * These are kept because ReactTableWrapper expects them.
   * They can be connected to your API later.
   */
  const [nextToken, setNextToken] = useState<string | null>(null);
  const [previousTokens, setPreviousTokens] = useState<string[]>([]);
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((item) => {
      return (
        ...
        (item.denergyWallet &&
          item.denergyWallet.toLowerCase().includes(search)) ||
        (item.ethereumWallet &&
          item.ethereumWallet.toLowerCase().includes(search)) ||
        (item.applicantId &&
          item.applicantId.toLowerCase().includes(search)) ||
        (item.reviewStatus &&
          item.reviewStatus.toLowerCase().includes(search))
      );
    });
  }, [searchTerm, data]);

  /**
   * Temporary pagination handler.
   *
   * No GraphQL / fetchMore is used.
   *
   * This keeps ReactTableWrapper working while we remove
   * GraphQL from the application.
   */
  const handlePagination = useCallback(
    async (direction: 'next' | 'previous' | 'first') => {
      switch (direction) {
            ...
            >
              ...
              >
                <div>
                  ...
                  {row.original.kycDetails?.fullResponse?.info?.firstName ||
                    'N/A'}
                </div>

                <div>
                  <strong>Last Name:</strong>{' '}
                  {row.original.kycDetails?.fullResponse?.info?.lastName ||
                    'N/A'}
                </div>

                <div>
                  <strong>Date of Birth:</strong>{' '}
                  {row.original.kycDetails?.fullResponse?.info?.dob || 'N/A'}
                </div>

                <div>
                  <strong>Country:</strong>{' '}
                  {row.original.kycDetails?.fullResponse?.info?.country ||
                    'N/A'}
                </div>

                <div>
                  <strong>Nationality:</strong>{' '}
                  {row.original.kycDetails?.fullResponse?.fixedInfo
                    ?.nationality || 'N/A'}
                </div>

                <div>
                  <strong>Email:</strong>{' '}
                  {row.original.kycDetails?.email || 'N/A'}
                </div>

                <div>
                  <strong>Review Status:</strong>{' '}
                  {row.original.kycDetails?.fullResponse?.review?.reviewResult
                    ?.reviewAnswer || 'N/A'}
                </div>

                <div>
                  <strong>IP Country:</strong>{' '}
                  {row.original.kycDetails?.ipCountry || 'N/A'}
                </div>
              </Box>
            </Box>
          )}
